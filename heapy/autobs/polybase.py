...
class PolyBase(object):
    ...
    def save(self, savepath, suffix=''):
        ...
        ax[0].plot(self.time, self.rate, lw=1.0, c='b', label='Light curve')
        ax[0].plot(self.time, self.base_res['bak'], lw=1.0, c='g', label='Baseline')
        ax[0].plot(self.time, self.bak, lw=1.0, c='r', label='Polynomial')
        ax[0].set_xlim([min(self.time), max(self.time)])
        ax[0].set_xlabel('Time')
        ax[0].set_ylabel('Rate')
        ax[0].minorticks_on()
        ax[0].tick_params(axis='x', which='both', direction='in', labelcolor='k', colors='k')
        ax[0].tick_params(axis='y', which='both', direction='in', labelcolor='k', colors='k')
        ax[0].tick_params(which='major', width=1.0, length=5)
        ax[0].tick_params(which='minor', width=1.0, length=3)
        ax[0].xaxis.set_ticks_position('both')
        ax[0].yaxis.set_ticks_position('both')
        ax[0].legend(frameon=False)

        p1, = ax[1].plot(self.time, self.rate - self.bak, lw=1.0, c='b', label='Net light curve')
        ax[1].set_xlim([min(self.time), max(self.time)])
        ax[1].set_xlabel('Time')
        ax[1].set_ylabel('Rate')
        ax[1].minorticks_on()
        ax[1].tick_params(axis='x', which='both', direction='in', labelcolor='k', colors='k')
        ax[1].tick_params(axis='y', which='both', direction='in', labelcolor='k', colors='k')
        ax[1].tick_params(which='major', width=1.0, length=5)
        ax[1].tick_params(which='minor', width=1.0, length=3)
        ax[1].xaxis.set_ticks_position('both')
        ax[1].yaxis.set_ticks_position('both')

        ax1 = ax[1].twinx()
        p3, = ax1.plot(self.edges, np.append(self.re_snr, [self.re_snr[-1]]), lw=1.0, c='r', 
                       label='Bayesian block SNR', drawstyle='steps-post')
        p4 = ax1.axhline(self.sigma, lw=1.0, c='g', ls='--', label='%.1f$\\sigma$' % self.sigma)
        ax1.set_xlim([min(self.time), max(self.time)])
        ax1.set_ylabel('SNR')
        ax1.set_ylim([-6, 26])

        ax[1].legend(handles=[p1, p3, p4], frameon=False)
        fig.savefig(savepath + '/bs%s.pdf'%suffix, bbox_inches='tight', pad_inches=0.1, dpi=300)
        plt.close(fig)